...
The previous manuscript is stored directly as a Git blob under tag `manuscript-pre-hermite-20260912`, so preserving it does not require another manuscript file. The updated manuscript is tagged `manuscript-hermite-20260912`. Both are retrievable using `git show TAG`; the sole working manuscript remains at its original path. TeX environments, braces and label references passed structural checks only, not compilation. Universal floating-point certification, independent held-out computation confirmation and venue-level novelty acceptance remain unproved.
